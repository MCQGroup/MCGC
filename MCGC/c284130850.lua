...
function c284130850.filter3(c, e, tp, m, f, chkf)
    -- 融合对象
    return c:IsType(TYPE_FUSION) and c284130850.filter(c) and(not f or f(c))
...
function c284130850.activateTarget(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
    if chk == 0 then
        ...
        local mg1 = Duel.GetMatchingGroup(c284130850.filter2, tp, LOCATION_MZONE, 0, nil, e)
        -- 场上可以用作融合素材的卡片
...
function c284130850.activateOperation(e, tp, eg, ep, ev, re, r, rp)
    local chkf = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
    local mg1 = Duel.GetMatchingGroup(c284130850.filter2, tp, LOCATION_MZONE, 0, nil, e)
    local sg1 = Duel.GetMatchingGroup(c284130850.filter3, tp, LOCATION_EXTRA, 0, nil, e, tp, mg1, nil, chkf)
    local mg2 = nil
    local sg2 = nil
    local ce = Duel.GetChainMaterial(tp)
    if ce ~= nil then
        ...
        mg2 = fgroup(ce, e, tp)
        local mf = ce:GetValue()
        sg2 = Duel.GetMatchingGroup(c284130850.filter3, tp, LOCATION_EXTRA, 0, nil, e, tp, mg2, mf, chkf)
    end
...
function c284130850.MCQFilter(c)
    return c:IsCode(284130814) and c:IsFaceup()
end

function c284130850.graveCondition(e, tp, eg, ep, ev, re, r, rp)
    return Duel.IsExistingMatchingCard(c284130850.MCQFilter, tp, LOCATION_SZONE, 0, nil)
end
...
function c284130850.graveFusionFilter(c)
    return c284130850.filter(c) and c:IsType(TYPE_FUSION)
end

function c284130850.graveOperation(e, tp, eg, ep, ev, re, r, rp)
    local sel = Duel.SelectOption(tp, aux.Stringid(284130850, 0), aux.Stringid(284130850, 1))
    Debug.Message(sel)
    if sel == 0 then
        local g = Duel.SelectMatchingCard(tp, c284130850.graveFusionFilter, tp, LOCATION_GRAVE, 0, 1, 1, nil)
        local c = g:GetFirst()
        local pos = Duel.SelectPosition(tp, c, POS_FACEUP)
        Duel.SpecialSummon(c, SUMMON_TYPE_SPECIAL, tp, tp, true, true, pos)
    else
        local g1 = Duel.GetMatchingGroup(c284130850.MCQFilter, tp, LOCATION_SZONE, 0, nil)
        ...
        if Duel.SpecialSummonStep(c, SUMMON_TYPE_SPECIAL, tp, tp, true, true, pos) then
            ...
            e2:SetOperation(c284130850.backExtraOperation)
            c:RegisterEffect(e2)
        end
    end

    function c284130850.backExtraOperation(e, tp, eg, ep, ev, re, r, rp)
        -- 卧槽应该怎么把卡放进额外卡组？
