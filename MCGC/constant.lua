...
MAX_ID = 268435455  -- 9 digit, by DataManager::GetDesc() == 0xFFF FFFF
...
LOCATION_REASON_TOFIELD     = 0x1   -- 默认值，凯萨竞技场
...
REASON_COST         = 0x80          -- 用于代价或无法支付代价而破坏
...
-- Summon Type -- 召唤类型
SUMMON_TYPE_NORMAL      = 0x10000000    -- 通常召唤(EFFECT_SUMMON_PROC,EFFECT_SET_PROC 可用Value修改数值)
SUMMON_TYPE_ADVANCE     = 0x11000000    -- 上级召唤
SUMMON_TYPE_DUAL        = 0x12000000    -- 再度召唤(二重)
SUMMON_TYPE_FLIP        = 0x20000000    -- 翻转召唤
SUMMON_TYPE_SPECIAL     = 0x40000000    -- 特殊召唤(EFFECT_SPSUMMON_PROC,EFFECT_SPSUMMON_PROC_G 可用Value修改数值)
...
STATUS_SPSUMMON_STEP        = 0x0080        -- 效果特召处理中
STATUS_FORM_CHANGED         = 0x0100        -- 改变过表示形式
STATUS_SUMMONING            = 0x0200        -- 召唤中
STATUS_EFFECT_ENABLED       = 0x0400        -- 效果有效
STATUS_SUMMON_TURN          = 0x0800        -- 在本回合召唤/SET
STATUS_DESTROY_CONFIRMED    = 0x1000        -- 破坏确定
STATUS_LEAVE_CONFIRMED      = 0x2000        -- 连锁处理完后送去墓地的魔法陷阱
STATUS_BATTLE_DESTROYED     = 0x4000        -- 战斗破坏确定后尚未移动
STATUS_COPYING_EFFECT       = 0x8000        -- 复制效果
STATUS_CHAINING             = 0x10000       -- 正在连锁串中
STATUS_SUMMON_DISABLED      = 0x20000       -- 召唤无效后尚未移动
STATUS_ACTIVATE_DISABLED    = 0x40000       -- 发动无效后尚未移动
...
STATUS_ACTIVATED            = 0x800000      -- 魔法陷阱卡发动过
STATUS_JUST_POS             = 0x1000000     -- 已改变表示形式(用于STATUS_CONTINUOUS_POS判定)
STATUS_CONTINUOUS_POS       = 0x2000000     -- 改变后再次设定成其他表示形式
STATUS_FORBIDDEN            = 0x4000000     -- 不能play
STATUS_ACT_FROM_HAND        = 0x8000000     -- 从手牌发动
STATUS_OPPO_BATTLE          = 0x10000000    -- 和对手的怪兽战斗
...
RESET_OVERLAY       = 0x04000000    -- 超量迭放重置
...
EFFECT_TYPE_ACTIONS     = 0x0008    -- 触发型，以下类型会自动添加此属性(对峙的G)
...
EFFECT_TYPE_CONTINUOUS  = 0x0800    -- 由事件触发的辅助用效果/永续效果
...
EFFECT_FLAG_SET_AVAILABLE       = 0x0100        -- 影响场上里侧的卡/里侧状态可发动
EFFECT_FLAG_CANNOT_NEGATE       = 0x0200        -- 含有"此效果不会被无效"的叙述
...
EFFECT_FLAG_COPY_INHERIT        = 0x2000        -- 若由复制的效果产生则继承其Reset属性
EFFECT_FLAG_DAMAGE_STEP         = 0x4000        -- 可以在伤害步骤发动
EFFECT_FLAG_DAMAGE_CAL          = 0x8000        -- 可以在伤害计算时发动
EFFECT_FLAG_DELAY               = 0x10000       -- 场合型诱发效果、用于永续效果的EFFECT_TYPE_CONTINUOUS、神之化身/恐惧之源的攻击力变化最后计算
EFFECT_FLAG_SINGLE_RANGE        = 0x20000       -- 只对自己有效
EFFECT_FLAG_UNCOPYABLE          = 0x40000       -- 不能复制(效果外文本)
EFFECT_FLAG_OATH                = 0x80000       -- 誓约效果
EFFECT_FLAG_SPSUM_PARAM         = 0x100000      -- 指定召唤/规则特殊召唤的位置和表示形式(熔岩魔神)
...
EFFECT_FLAG_OWNER_RELATE        = 0x1000000     -- 持续成为对象
...
EFFECT_IGNORE_BATTLE_TARGET         = 72        -- 不能成为攻击对象-鶸型(传说的鱼人)
...
EFFECT_SET_ATTACK                   = 101       -- 设置攻击力(永续型效果、攻击力变成X特殊召唤)
EFFECT_SET_ATTACK_FINAL             = 102       -- 设置最终攻击力(所有入连锁的改变攻击力)
...
EFFECT_SET_POSITION                 = 140       -- 设定表示形式
...
EFFECT_TOSS_DICE_REPLACE            = 221       -- 重新掷股子
EFFECT_FUSION_MATERIAL              = 230       -- 指定融合素材的条件
EFFECT_CHAIN_MATERIAL               = 231       -- 玩家受到连锁物质的效果影响
...
EFFECT_MAX_MZONE                    = 263       -- 怪兽区格数上限
EFFECT_MAX_SZONE                    = 264       -- 魔陷区格数上限
...
EFFECT_TO_GRAVE_REDIRECT_CB         = 313       -- 重新指定去向(宝玉兽)
EFFECT_CHANGE_LEVEL_FINAL           = 314       -- 设定最终等级(银河女王之光)
EFFECT_CHANGE_RANK_FINAL            = 315       -- 设定最终阶级
EFFECT_SPSUMMON_PROC_G              = 320       -- P召唤规则
EFFECT_SPSUMMON_COUNT_LIMIT         = 330       -- 特殊召唤次数限制
EFFECT_LEFT_SPSUMMON_COUNT          = 331       -- 剩余召唤次数(召唤限制网)
EFFECT_CANNOT_SELECT_BATTLE_TARGET  = 332       -- 对手不能选择为攻击对象
EFFECT_CANNOT_SELECT_EFFECT_TARGET  = 333       -- 对手不能选择为效果对象
EFFECT_ADD_SETCODE                  = 334       -- 规则上视为「XX」字段
EFFECT_NO_EFFECT_DAMAGE             = 335       -- 玩家已受到"效果伤害变成0"的效果影响
EFFECT_UNSUMMONABLE_CARD            = 336       -- 不能通常召唤的怪兽
EFFECT_DISABLE_CHAIN_FIELD          = 337       -- 连锁串中场上发动的效果无效(Duel.NegateRelatedChain())
EFFECT_DISCARD_COST_CHANGE          = 338       -- 反制陷阱舍弃手牌的代价改变(解放之阿里阿德涅)
...
EVENT_CHAIN_ACTIVATING      = 1021      -- 连锁处理淮备中
...
EVENT_TOSS_DICE             = 1150      -- 掷股子时
EVENT_TOSS_COIN             = 1151      -- 抛硬币时
EVENT_TOSS_COIN_NEGATE      = 1152      -- 抛硬币被无效时
EVENT_TOSS_DICE_NEGATE      = 1153      -- 掷股子被无效时
...
TIMING_STANDBY_PHASE    = 0x2       -- 淮备阶段时点
...
TIMING_BATTLE_STEP_END  = 0x4000000 --战斗步骤结束时
---- 组合时点
...
GLOBALFLAG_SELF_TOGRAVE         = 0x100 -- 不入连锁的送墓检查(EFFECT_SELF_TOGRAVE)
GLOBALFLAG_SPSUMMON_ONCE        = 0x200 -- 1回合只能特殊召唤1次(Card.SetSPSummonOnce())
...
EFFECT_COUNT_CODE_SINGLE    = 0x1           -- 同一张卡的多个效果公共使用次数，不与其它code迭加使用
...
CARD_TWINKLE_MOSS   = 13857930  --光辉苔藓