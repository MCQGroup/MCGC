...
function c284130860.tg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
    if chkc then
        return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c284130860.filter(chkc, e, tp)
    end
    if chk == 0 then
        return Duel.IsExistingTarget(c284130860.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
    end
