...
function c84130838.initial_effect(c)
    ...
    e3:SetCondition(c84130838.ignitionCondition)
    e3:SetCost(c84130838.ignitionCost)
    e3:SetOperation(c84130838.ignitionOperation)

    local e4 = Effect.CreateEffect(c)
    e4:SetType(EFFECT_TYPE_CONTINUOUS)
    e4:SetCode(EVENT_PHASE_START + PHASE_STANDBY)
    e4:SetCondition(c84130838.refreshCondition)
    e4:SetOperation(c84130838.refreshOperation)
    c:RegisterEffect(e4)
...
function c84130838.ignitionCondition(e, tp, eg, ep, ev, re, r, rp)
    local isRightTurn = Duel.GetTurnPlayer() == tp
    local phase = Duel.GetCurrentPhase()
    local isRightPhase = phase == PHASE_MAIN1 or phase == PHASE_MAIN2
    local isAbleToSpSummon = e:GetLabelObject():GetLabel() == 0 and e:GetHandler():IsLocation(LOCATION_MZONE) and Duel.IsExistingMatchingCard(c84130838.spsummonFilter, tp, LOCATION_HAND, 0, 1, nil, e, SUMMON_TYPE_SPECIAL, tp, false, false)
...
function c84130838.ignitionCost(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then
        return true
    end
    local isAbleToSpSummon = e:GetLabelObject():GetLabel() == 0 and e:GetHandler():IsLocation(LOCATION_MZONE) and Duel.IsExistingMatchingCard(c84130838.spsummonFilter, tp, LOCATION_HAND, 0, 1, nil, e, SUMMON_TYPE_SPECIAL, tp, false, false)
    ...
    if isAbleToSpSummon and isAbleToSearch then
        option = Duel.SelectOption(tp, aux.Stringid(84130838, 0), aux.Stringid(84130838, 1))
...
function c84130838.ignitionOperation(e, tp, eg, ep, ev, re, r, rp)
...
function c84130838.refreshCondition(e, tp, eg, ep, ev, re, r, rp)
    return Duel.GetTurnPlayer() == tp
end

function c84130838.refreshOperation(e, tp, eg, ep, ev, re, r, rp)
    e:SetLabel(0)
end
