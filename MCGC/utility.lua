...
function Auxiliary.FConditionCode3(code1, code2, code3, sub, insf)
    return function(e, g, gc, chkfnf)
        ...
        if gc then
            ...
            local tc = mg:GetFirst()
            while tc do
                if tc:IsFusionCode(code1) then
                    b1 = 1
                elseif tc:IsFusionCode(code2) then
                    b2 = 1
                elseif tc:IsFusionCode(code3) then
                    b3 = 1
                end
                tc = mg:GetNext()
            end
            if gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE) then
                ...
                if gc:IsFusionCode(code1) then
                    b1 = 1
                elseif gc:IsFusionCode(code2) then
                    b2 = 1
                elseif gc:IsFusionCode(code3) then
                    b3 = 1
                else
                    return false
                end
    ...
    return function(e, g, gc, chkfnf)
        ...
        if gc then
            ...
            while tc do
                ...
                elseif tc:IsFusionCode(code3) then
                    b3 = 1
                elseif tc:IsFusionCode(code4) then
                    b4 = 1
                else
                    return false
                end
            ...
            if gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE) then
                ...
                elseif gc:IsFusionCode(code3) then
                    b3 = 1
                elseif gc:IsFusionCode(code4) then
                    b4 = 1
                end
...
function Auxiliary.FOperationCode4(code1, code2, code3, code4, sub, insf)
    return function(e, tp, eg, ep, ev, re, r, rp, gc, chkfnf)
        ...
        if g3:GetFirst():IsHasEffect(EFFECT_FUSION_SUBSTITUTE) then
            ...
            sg:Remove(Card.IsFusionCode, nil, g3:GetFirst():GetCode())
        end
        Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_FMATERIAL)
...
function Auxiliary.FConditionFun2(f1, f2, insf)
    return function(e, g, gc, chkfnf)
        ...
        if gc then
            ...
            or(f2(gc) and mg:IsExists(f1, 1, gc))
        end
...
function Auxiliary.FConditionCodeRep(code, cc, sub, insf)
    return function(e, g, gc, chkfnf)
        ...
        if gc then
            if not gc:IsCanBeFusionMaterial(e:GetHandler()) then return false end
            return(gc:IsFusionCode(code) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and mg:IsExists(Card.IsFusionCode, cc - 1, gc, code)
        end
...
function Auxiliary.FConditionFunRep(f, cc, insf)
    return function(e, g, gc, chkfnf)
        ...
        if gc then
            if not gc:IsCanBeFusionMaterial(e:GetHandler()) then return false end
            return f(gc) and mg:IsExists(f, cc - 1, gc)
        end
