...
function c84130833.initial_effect(c)
    -- 一回合只使用一次
    -- 召唤触发抉择
    ...
    e1:SetCost(c84130833.cost)
    e1:SetOperation(c84130833.operation)
    e1:SetCountLimit(1, 84130833)
    c:RegisterEffect(e1)
end

function c84130833.cost(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then
    ...
    if Duel.IsExistingMatchingCard(aux.TRUE, 1 - tp, LOCATION_ONFIELD, 0, 1, nil) then
        sel = Duel.SelectOption(tp, aux.Stringid(84130833, 0), aux.Stringid(84130833, 1))
    else
        sel = 0
    end
    Duel.SetOperationInfo(0x2222, 84130833, nil, g:GetCount(), nil, sel)
    Duel.SendtoDeck(g, nil, 2, REASON_COST)
end

function c84130833.operation(e, tp, eg, ep, ev, re, r, rp)
    local test, g, count, p, sel = Duel.GetOperationInfo(0x2222, 84130833)
    if test then
        if sel == 0 then
